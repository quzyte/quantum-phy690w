...
! NX, NY : Number of space mesh points (X and Y)  
  INTEGER, PARAMETER :: NX = 500, NXX = NX-1 ! No. of X steps
  INTEGER, PARAMETER :: NY = 400, NYY = NY-1, NY2 = NY/2 ! No. of Y steps
...
! NRUN : Number of final time iterations with fixed nonlinearity
  INTEGER, PARAMETER :: NPAS = 100000, NRUN = 20000
END MODULE COMM_DATA
MODULE GPE_DATA
  USE COMM_DATA, ONLY : NX, NY
  REAL (8), PARAMETER :: PI = 3.14159265358979D0
! DX, DY : Space step and DT : Time step
  REAL (8), PARAMETER :: DX = 0.02D0, DY = 0.02D0, DT = 0.00004D0
...
PROGRAM GROSS_PITAEVSKII_SSCN_AXIAL
  USE COMM_DATA, ONLY : NX, NY, NY2, NPAS, NRUN
  USE GPE_DATA
  IMPLICIT NONE
...
!------------------------ END INTERFACE BLOCKS -------------------
  INTEGER :: I, J, K
  REAL (8) :: ZNORM, RHORMS, ZRMS, RRMS, MU, EN
  SELECT CASE (OPTION)
    CASE (1,3)
      XOP = 1.0D0
    CASE (2)
  ...
  WRITE (7, 1002)
  WRITE (7, 1001)
  WRITE (7, 1003) ZNORM, MU/XOP, EN/XOP, RHORMS, ZRMS, RRMS, CP(0,NY2+1)
  1001 FORMAT(12X,65('-'))
  1002 FORMAT(14X,'Norm',4X,'Chem',5X,'Energy',4X,'<rho>',5X,'<z>', &
       7X,'<r>',7X,'psi(0)')
  1003 FORMAT ('Initial   :', F6.3, 6(F10.5))
!
  DO I = 0, NX, 10 ! Writes initial wave function
     DO J = 0, NY, 10
...
!
  ...
  CALL CHEM(CP, MU, EN)
  CALL RAD(CP, RHORMS, ZRMS, RRMS)
  WRITE(7,1005) ZNORM, MU/XOP, EN/XOP, RHORMS, ZRMS, RRMS, CP(0,NY2+1)
...
! X and Y, respectively, using Crank-Nicholson scheme.
  ...
  CALL CHEM(CP, MU, EN)
  CALL RAD(CP, RHORMS, ZRMS, RRMS)
  WRITE(7,1004) ZNORM, MU/XOP, EN/XOP, RHORMS, ZRMS, RRMS, CP(0,NY2+1)
  WRITE (7, 1001)
...
! derivative term using the Crank-Nicholson method
  ...
  REAL (8), DIMENSION(0:,0:), INTENT(INOUT) :: CP
  INTEGER :: I, J
  REAL (8), DIMENSION (0:NX, 0:NY) :: CBE
  REAL (8), DIMENSION (0:NY) :: CXX
  DO J = 0, NY
     CBE(1,J) = 0.0D0
     DO I = 1, NXX
        CXX(J) = CP(I,J)-CAIPMX*(CP(I+1,J)-2.0D0*CP(I,J)+CP(I-1,J)) &
                - CPX(I)*(CP(I+1,J)-CP(I-1,J))
        CBE(I+1,J) = CGAX(I)*(CAIPX(I)*CBE(I,J)-CXX(J))
     END DO
     CP(NX,J) = 0.0D0
     DO I = NX, 1, -1
...
!  derivative term using the Crank-Nicholson method
  ...
  DO I = 0, NX
     CBE(I,NYY) = CP(I, NY)
     DO J = NYY, 1, -1
        CYY(I) = CP(I,J)-CAIPMY*(CP(I,J+1)-2.0D0*CP(I,J)+CP(I,J-1))
        CBE(I,J-1) = CGAY(J)*(CAIPMY*CBE(I,J)-CYY(I))
     END DO
     DO J = 0, NY-1
