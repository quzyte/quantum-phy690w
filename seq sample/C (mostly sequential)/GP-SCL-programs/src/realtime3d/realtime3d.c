...
int main(int argc, char **argv) {
   ...
   cbeta =  alloc_complex_vector(MAX(Nx, Ny, Nz) - 1);
   ...
   fflush(out);
   if(Nstpout != NULL) {
      sprintf(filename, "%s.x", Nstpout);
      file = fopen(filename, "w");
      for(cnti = 0; cnti < Nx; cnti += outstpx)
         fprintf(file, "%8le %8le\n", x[cnti], cabs(psi[cnti][Ny2][Nz2]));
      fclose(file);
      
      sprintf(filename, "%s.y", Nstpout);
      file = fopen(filename, "w");
      for(cnti = 0; cnti < Ny; cnti += outstpy)
         fprintf(file, "%8le %8le\n", y[cnti], cabs(psi[Nx2][cnti][Nz2]));
      fclose(file);
      
      sprintf(filename, "%s.z", Nstpout);
      file = fopen(filename, "w");
      for(cnti = 0; cnti < Nz; cnti += outstpz)
         fprintf(file, "%8le %8le\n", z[cnti], cabs(psi[Nx2][Ny2][cnti]));
      fclose(file);
...
void calcmuen(double *mu, double *en, double complex ***psi, double ***dpsix, double ***dpsiy, double ***dpsiz, double *tmpxi, double *tmpyi, double *tmpzi, double *tmpxj, double *tmpyj, double *tmpzj) {
   ...
   for(cntj = 0; cntj < Ny; cntj ++) {
      for(cntk = 0; cntk < Nz; cntk ++) {
         for(cnti = 0; cnti < Nx; cnti ++) {
            tmpxi[cnti] = cabs(psi[cnti][cntj][cntk]);
         }
   ...
   for(cnti = 0; cnti < Nx; cnti ++) {
      for(cntk = 0; cntk < Nz; cntk ++) {
         for(cntj = 0; cntj < Ny; cntj ++) {
            tmpyi[cntj] = cabs(psi[cnti][cntj][cntk]);
         }
   ...
   for(cnti = 0; cnti < Nx; cnti ++) {
      for(cntj = 0; cntj < Ny; cntj ++) {
         for(cntk = 0; cntk < Nz; cntk ++) {
            tmpzi[cntk] = cabs(psi[cnti][cntj][cntk]);
         }
 ...
 *    tmpz - temporary array
 */
void calcrms(double *rms, double complex ***psi, double *tmpx, double *tmpy, double *tmpz) {
   long cnti, cntj, cntk;
   double psi2;
