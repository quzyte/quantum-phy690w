...
int main(int argc, char **argv) {
   ...
   fprintf(out, " Real time propagation spherically-symmetric trap,   OPTION = %d\n\n", opt);
   ...
   fprintf(out, "                  --------------------------------------------------------\n");
   fprintf(out, "                    Norm      Chem        Ener/N     <r>     |Psi(0)|^2\n");
   fprintf(out, "                  --------------------------------------------------------\n");
   fflush(out);  
   ...
   if(initout != NULL) {
      ...
      for(cnti = 1; cnti < Nr; cnti += outstpr)
         fprintf(file, "%8le %8le\n", r[cnti], cabs(psi[cnti]) / r[cnti] * cabs(psi[cnti]) / r[cnti]);
      fclose(file);
   }
   if(Nstp != 0) {  
...
void init(double complex *psi, double *abc) {
   ...
   if(Nstp==0  ) {
     if((file = fopen("imagsph-den.txt", "r"))==NULL) {       /* open a text file for reading */
       printf("Run the program using the input file to read. i.g.: imagsph-den.txt\n");
       exit(1);					/*couldn't open the requested file!*/
     }
     file = fopen("imagsph-den.txt", "r");
     for(cnti = 0; cnti < Nr; cnti ++) {
